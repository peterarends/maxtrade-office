  ...
  <div class="tasks-body">
    <div class="button-bar" :class="getTheme">
      <div class="topTitleDiv">
        ...
        <span>
          {{ t("Task") }}:
          {{ getTask.id | formatTaskId(getNewTask) }}
        </span>
      </div>
    ...
    <div class="body" :class="[getTask.status == 0 ? 'ended' : '']">
      <div class="date">
        <h3>{{ t("Date start") }}: {{ getTask.created_at | formatDate }}</h3>
        <h3>
          {{ t("Date end") }}:
      ...
      <div class="taskStatus">
        <input type="checkbox" id="task_status" v-model="getTask.status" />
        <label for="task_status">
          <span></span>
          {{ t("Status of the task") }}:
          <strong>
            {{
            getTask.status
      ...
      <div class="documents" :class="getTheme">
        <div class="document" v-for="document in getDocuments" :key="document">
          <a
            target="_blank"
            v-bind:href="
      ...
      <div class="documentsButtons">
        ...
        <button class="button" :title="t('Add selected file.')" @click="onUpload">
    ...
    <div class="bottom" :class="getTheme">
      <a @click.prevent="saveTask(true)">
        <i class="mdi mdi-content-save-outline mdiTaskIcon" :class="getTheme"></i>
        &nbsp;{{ t("Save") }}
      </a>
      <a @click.prevent="deleteTask">
        <i class="mdi mdi-delete-outline mdiTaskIcon" :class="getTheme"></i>
        &nbsp;{{ t("Delete") }}
      </a>
      <a @click.prevent="closePanel">
        <i class="mdi mdi-close-outline mdiTaskIcon" :class="getTheme"></i>
        &nbsp;{{ t("Close") }}
      </a>
        ...
        .post("api/task/file/" + this.getTask.id, formData, {
          onUploadProgress: uploudEvent => {
            this.$refs.uploadPurcent.innerHTML =
              Math.round((uploudEvent.loaded / uploudEvent.total) * 100) + "%";
            this.changeProgress(
              Math.round((uploudEvent.loaded / uploudEvent.total) * 100)
            );
          }
